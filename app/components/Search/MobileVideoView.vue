<template>
    <v-sheet class="fill-height overflow-y-auto bg-transparent">
        <div v-if="pending" class="d-flex justify-center align-center fill-height">
            <v-progress-circular indeterminate color="primary" />
        </div>

        <div v-else-if="flatVideoList.length === 0" class="d-flex justify-center align-center fill-height">
        ...
        <div v-else class="pa-2">
            ...
            >
                <div class="d-flex align-center mb-2 px-2">
                    ...
                    <span class="text-subtitle-1 font-weight-bold">{{ business.biz_name }}</span>
                </div>
...
const handleVideoSelect = (videoId: string, toggle: () => void, isSelected: boolean) => {
...
const groupedByBusiness = computed(() => {
    const groups: Record<string, { biz_name: string; biz_slug: string; videos: typeof flatVideoList.value }> = {};
...
.title-overlay {
    ...
    -webkit-backdrop-filter: blur(8px); /* Compatibilité avec les navigateurs basés sur WebKit (Safari) */
