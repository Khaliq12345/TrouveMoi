<template>
  <v-container class="pa-0 py-8">
    <div class="d-flex align-center justify-space-between mb-6 px-4">
      <div>
        <h2 class="text-h4 font-weight-bold mb-1">C'est quoi l'ambiance ?</h2>
        <p class="text-body-2 text-grey-darken-1">
          Explorez l'atmosphère à travers nos galeries
        </p>
      </div>
      <v-btn
        v-if="hasVibes"
    ...
    <v-row class="px-2">
      <v-col v-for="group in groupedVibes" :key="group.title" cols="12" sm="4">
        <v-card 
        ...
        >
          ...
          >
            <div class="vibe-overlay pa-6 w-100">
              ...
              <p class="text-body-2 text-white-70">{{ group.totalImages }} photos</p>
            </div>
          </v-img>
        </v-card>
      </v-col>
...
const hasVibes = computed(() => (metas?.value?.vibes?.length || 0) > 0);

// Logique de groupement avec déduplication stricte
const groupedVibes = computed(() => {
  const vibes = metas?.value?.vibes || [];
  const groups: Record<string, any> = {};

  vibes.forEach((item: any) => {
    ...
    const isDuplicateItem = groups[key].items.some((existingItem: any) => existingItem.id === item.id);
    if (!isDuplicateItem) {
...
const openFullModal = () => {
  ...
  modalItems.value = metas?.value?.vibes || [];
  isModalOpen.value = true;
};
</script>
